...
/**
 * deploys the required components to a simplified web framework, that can be accessed
 * by injecting a MockMvc reference
 */
@WebMvcTest(EmployeeRestController.class)
class EmployeeController_WithMockServiceIT {

    @Autowired
    private MockMvc mvc;

    // mocking required beans limits the components to load
    @MockBean
    ...
    void whenPostEmployee_thenCreateEmployee( ) throws Exception {
        ...
        mvc.perform(
                post("/api/employees").contentType(MediaType.APPLICATION_JSON).content(JsonUtil.toJson(alex)))
