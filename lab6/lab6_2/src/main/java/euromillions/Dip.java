...
public class Dip {
    ...
    private static final int NUM_OF_STARTS = 2;
    private static final int NUMBER_RANGE_MAX = 50;
    private static final int START_RANGE_MAX = 12;
    ...
    public Dip(int[] arrayOfNumbers, int[] arrayOfStarts) {
        ...
        if (NUM_OF_NUMBERS == arrayOfNumbers.length && NUM_OF_STARTS == arrayOfStarts.length) {
        ...
        for (int n : arrayOfNumbers) {
            if (n > NUMBER_RANGE_MAX) throw new IllegalArgumentException("max range exceded in number values");
        }
        for (int n : arrayOfStarts) {
            if (n > START_RANGE_MAX) throw new IllegalArgumentException("max range exceded in star values");
        }
    }
    ...
    public static Dip generateRandomDip() {
        ...
        for (int i = 0; i < NUM_OF_NUMBERS; ) {
            int candidate = generator.nextInt(NUMBER_RANGE_MAX-1) + 1;
            if (!randomDip.getNumbersColl().contains(candidate)) {
                randomDip.getNumbersColl().add(candidate);
                i++;
            }
        }
        for (int i = 0; i < NUM_OF_STARTS; ) {
            int candidate = generator.nextInt(START_RANGE_MAX-1) + 1;
